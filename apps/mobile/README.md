...
The default API is `http://localhost:8787`, or `http://10.0.2.2:8787` on the Android emulator. Set `EXPO_PUBLIC_API_URL` to your reachable server URL for a physical device or deployment. Live mode asks for the server access key; sample mode opens the fictional workspace automatically. Tokens stay in memory.
...
- Chat restores/saves AG-UI conversation messages, renders frontend tool cards, and supports interruption, retry, and document references. A sample runtime is clearly identified.
